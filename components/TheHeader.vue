<template>
  <header class="bg-white fixed top-0 left-0 right-0 header" :class="showNavbar ? '' : 'is-fixed'">
    <div class="container">
      <div class="flex justify-between header__wrap" :class="isOpen ? 'relative z-10': 'static'">
        <div class="flex items-center justify-between">
          <nuxt-link to="/" class="header-logo" :class="scrollPosition > 60 ? 'logo-icon' : ''">
            <div>
              <img
                width="192"
                height="32"
                class="h-6 md:h-8 w-auto max-w-none"
                :src="isOpen ? 'images/vepaar-logo-gray.png' : 'images/vepaar-logo.png'"
                alt="vepaar-logo"
              />
            </div>
          </nuxt-link>
        </div>
        <div class="flex  justify-between lg:flex-1 items-center lg:bg-white relative">
          <TheNavigation :class="isOpen ? 'flex -z-[1] is-open' : ''" class="navigation"/>
