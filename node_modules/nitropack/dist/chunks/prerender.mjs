...
import { withLeadingSlash, withoutTrailingSlash, withBase, joinURL, withTrailingSlash, hasProtocol } from 'ufo';
...
async function prerender(nitro) {
  ...
  const generateRoute = async (route) => {
    ...
    if (nitro.options.prerender.crawlLinks && isImplicitHTML) {
      return new Set(extractLinks(contents));
    }
  };
  function createTasks(routes2) {
    const tasks2 = new Listr([], { concurrent: 1, exitOnError: false, collapse: false });
